...
export default function Carousel() {
  ...
  const visibleStartIndex = Math.max(0, currentIndex - Math.floor(maxVisible / 2));
    ...
    <div className="relative w-full flex flex-col justify-center items-center min-h-screen gap-2 mx-auto px-4">
          ...
          {timages.map((item, index) => (
            <div key={index} className="w-full flex-shrink-0 flex flex-col items-center justify-center">
              <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6 w-full">
                {item.type === "image" ? (
                  <img src={item.url} alt="Slide Image" className="w-[400px] max-w-[400px]  md:max-w-[600px] lg:max-w-[800px] object-contain" />
                ) : item.type === "video" ? (
                  <video
                ...
                {item.description && (
                  <div className="max-w-xs text-left px-4">
                    <p className="text-gray-700" dangerouslySetInnerHTML={{ __html: item.description }} />
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="flex flex-col items-center justify-center mt-5 w-full">
          ...
          {visibleImages.map((image, index) => {
            ...
            return (
              <div key={imageIndex} className="text-center">
                <p className={`text-sm md:text-lg font-semibold mb-5 ${isCenterDot ? "text-gray-800" : "text-gray-500"}`}>
                  {image.year}
                </p>

                <span
                  onClick={() => goToImage(imageIndex)}
                  className={`cursor-pointer block w-3 h-3 md:w-4 md:h-4 lg:w-5 lg:h-5 rounded-full mx-auto transition-all duration-300 ${isCenterDot ? "bg-orange-600 scale-[2] border-2 border-[#C9A65C]" : "bg-orange-600 opacity-75"
                    }`}
                ></span>
              </div>
            );
        ...
        <div
          ...
          style={{
            background: "linear-gradient(to right, transparent, rgba(139, 116, 57, 0.7), transparent)",
          }}
        ></div>
      </div>
